...
# # Структура финальных требований
...
#     special_requirements: str = Field(description="Особые условия (безопасность, офлайн и т.д.)")
...
# Если же информации достаточно — **немедленно верни структурированные данные в JSON**,
# соответствующие схеме Requirements.

# Не объясняй, не пиши лишнего — только вопрос ИЛИ валидный JSON.
# """),
#     MessagesPlaceholder(variable_name="chat_history"),
# ])

# # Создаём цепочку с ожиданием либо текста (вопроса), либо JSON
# chat_chain = prompt | llm

# # Отдельная цепочка для финального JSON
# json_chain = prompt | llm.with_structured_output(Requirements)
...
#         # Сначала попробуем получить JSON
#         try:
#             response_obj = json_chain.invoke({"chat_history": chat_history})
#             # Если получили объект — завершаем
#             print("\n✅ Формализованные требования:")
#             print(f"- Цель: {response_obj.goal}")
#             print("- Основные функции:")
#             for i, feat in enumerate(response_obj.features, 1):
#                 print(f"  {i}. {feat}")
#             print(f"- Аудитория: {response_obj.audience}")
#             print(f"- Особые требования: {response_obj.special_requirements}")
#             break
#         except Exception as e:
#             # Если не удалось распарсить JSON — значит, LLM задаёт вопрос
#             response = chat_chain.invoke({"chat_history": chat_history})
